...
export default function GradeManagement({ tutorId }: { tutorId: string }) {
  ...
  return (
    <div className="max-w-full sm:max-w-6xl mx-auto px-4 sm:px-8 py-8 h-screen overflow-y-auto overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10 flex-shrink-0">
        <div className="glass-card rounded-2xl p-6 border-2 border-white/30 shadow-glow-lg hover-lift">
          <p className="text-gray-300 text-sm">Total Students</p>
          <p className="text-2xl font-bold text-white">{totalStudents}</p>
        </div>
        <div className="glass-card rounded-2xl p-6 border-2 border-green-400/30 shadow-glow-lg hover-lift">
          <p className="text-green-300 text-sm">Fully Graded</p>
          <p className="text-2xl font-bold text-white">{fullyGradedStudents}</p>
        </div>
        <div className="glass-card rounded-2xl p-6 border-2 border-yellow-400/30 shadow-glow-lg hover-lift">
          <p className="text-yellow-300 text-sm">In Progress</p>
          <p className="text-2xl font-bold text-white">{partiallyGradedStudents}</p>
        </div>
        <div className="glass-card rounded-2xl p-6 border-2 border-red-400/30 shadow-glow-lg hover-lift">
      ...
      <div className="mb-8 flex-shrink-0">
        <div className="glass-card rounded-2xl p-6 border-2 border-white/30 shadow-glow-lg">
          <div className="relative">
            <input
              ...
              className="w-full px-6 py-4 bg-white/10 border border-white/20 rounded-xl text-white placeholder-gray-400 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            {searchTerm && (
              <button
                onClick={() => setSearchTerm('')}
                className="absolute right-4 top-4 px-3 py-2 bg-red-600/20 text-red-300 rounded-lg text-sm hover:bg-red-600/30 border border-red-400/30"
              >
                Clear
              </button>
            )}
      ...
      {loading ? (
        ...
        <div className="grid gap-8">
          {filteredStudents.map(student => (
            <div
              key={student.id}
              className="glass-card rounded-2xl p-6 border-2 border-white/30 shadow-glow-lg hover-lift transition-all duration-300"
            >
              {/* Student Header */}
              ...
              <div className="space-y-3 mb-6">
                {student.courses.map((course, index) => (
                  <div key={index} className="glass-card rounded-lg p-4 border border-white/20">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <select
                          ...
                          className="w-32 px-3 py-2 bg-white/20 border border-white/30 rounded-lg text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm cursor-pointer"
                        >
                          <option value="" className="bg-gray-900 text-gray-400">Select Grade</option>
                          {gradeOptions.map(grade => (
                            <option key={grade} value={grade} className="bg-gray-900 text-white hover:bg-blue-600">
                              {grade}
                            </option>
                          ))}
                        </select>
