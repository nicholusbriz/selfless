...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        if (oldDayRegistrations === 4) {
          throw new Error(
            'Cannot leave this day - it has exactly 4 students (minimum required). A day must have at least 4 students.',
          );
        }
    ...
    if (message.includes('closed for registration')) {
      return NextResponse.json({ error: message }, { status: 400 });
    }
    ...
    if (message.includes('minimum required')) {
      return NextResponse.json({ error: message }, { status: 400 });
    }
