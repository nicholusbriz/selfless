...
export async function POST(request: Request) {
  try {
    ...
    const totalCredits = courses.reduce((sum: number, course: any) => {
      if (!course.name || !course.credits) {
        throw new Error('All courses must have name and credits');
      }
      return sum + parseInt(course.credits);
    }, 0);
    ...
    return NextResponse.json({ 
      success: false, 
      message: error.message || 'Failed to save course registration' 
    }, { status: 500 });
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ 
      success: false, 
      message: error.message || 'Failed to retrieve course registration' 
    }, { status: 500 });
  }
}
